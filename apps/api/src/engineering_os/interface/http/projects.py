...
def create_project(body: ProjectCreate, db: Session = Depends(get_db)) -> ProjectOut:
    return _to_out(db, ProjectService(db).create(body.title, body.idea))


@router.get("/projects", response_model=list[ProjectOut])
def list_projects(db: Session = Depends(get_db)) -> list[ProjectOut]:
    return [_to_out(db, p) for p in ProjectService(db).list()]


@router.get("/projects/{project_id}", response_model=ProjectOut)
def get_project(project_id: str, db: Session = Depends(get_db)) -> ProjectOut:
    try:
        return _to_out(db, ProjectService(db).get(project_id))
